...
use App\Domain\Share\Repository\ShareRepositoryInterface;
...
class ShareRepository extends ServiceEntityRepository implements ShareRepositoryInterface
{
    ...
    public function getStatistics(): array
    {
        return $this->createQueryBuilder('s')
            ...
            ->addSelect('COUNT(CASE WHEN s.shareType = :fork THEN s.id END) as forkShares')
            ->addSelect('COUNT(CASE WHEN s.isPasswordProtected = true THEN s.id END) as passwordProtected')
            ->addSelect('SUM(s.viewCount) as totalViews')
            ->setParameter('now', new \DateTime())
            ->setParameter('view', Share::TYPE_VIEW)
            ->setParameter('edit', Share::TYPE_EDIT)
            ->setParameter('fork', Share::TYPE_FORK)
            ->getQuery()
            ->getSingleResult();
    }
